    ...
    public class OpenApiSpecComparator
    {
        ...
        private static void CompareVersions(ComparisonContext<OpenApiDocument> context, string oldVersion, string newVersion)
        {
            ...
            if (!context.Strict && oldVersionAsList.Length > 0 && newVersionAsList.Length > 0)
            {
                ...
                if (areIntegers && oldMajor != newMajor)
                {
                    ...
                    {
                        context.LogError(ComparisonMessages.VersionsReversed, oldVersionAsList, newVersionAsList);
                    }
                }
            ...
            IList<OpenApiServer> newServers)
        {
            ...
            foreach (var oldServerUrl in oldServerUrls.Except(newServerUrls))
            {
                ...
                context.LogBreakingChange(ComparisonMessages.ServerNoLongerSupported, oldServerUrl);
                context.Pop();
            }
            context.Pop();
        }
            ...
            bool isFromExtension = false)
        {
            ...
            foreach (var removedPath in oldPaths.Keys.Except(commonPaths))
            {
                ...
                context.LogBreakingChange(ComparisonMessages.RemovedPath, removedPathWithVariables);
                context.Pop();
            }
            ...
            IDictionary<OperationType,OpenApiOperation> newOperations)
        {
            ...
            foreach (var removedOperationName in oldOperations.Keys.Except(commonOperations))
            {
                ...
                context.LogBreakingChange(ComparisonMessages.RemovedOperation, oldOperations[removedOperationName].OperationId);
                context.Pop();
            }
        ...
        private void TrackSchemasReference(OpenApiDocument document)
             ...
             IDictionary<string, OpenApiParameter> newParameters)
         {
             ...
             foreach (var oldParameterName in oldParameters.Keys)
             {
                 ...
                 {
                     context.LogBreakingChange(ComparisonMessages.RemovedClientParameter, oldParameterName);
                 }
                 else
                 {
             ...
             IDictionary<string, OpenApiResponse> newResponses)
         {
             ...
             foreach (var oldDefinition in oldResponses.Keys)
             {
                 ...
                 {
                     context.LogBreakingChange(ComparisonMessages.RemovedDefinition, oldDefinition);
                 }
                 else
                 {
